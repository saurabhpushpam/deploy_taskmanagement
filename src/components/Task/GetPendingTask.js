...
const GetPendingTask = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        const response = await axios.get('https://taskmanagement-backend-97sv.onrender.com/api/getpendingtask', {
  ...
  const handleMarkAsComplete = async (taskId) => {
    try {
      await axios.post(`https://taskmanagement-backend-97sv.onrender.com/api/updatetask/${taskId}`,
        { status: 'Complete' },
        { headers: { Authorization: `${token}` } }
      );
